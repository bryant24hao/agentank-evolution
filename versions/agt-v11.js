function onIdle(me, enemy, game) {
  ...
  // v11: break long straight lines so blind-firing opponents can't predict our path
  if (next && shouldBreakStraightLine(myPos, myDir, next, map, enemyTank, enemy)) {
    var sidestepDir = pickJukeDir(myPos, myDir, map, enemyTank);
    if (sidestepDir) {
      queueFace(me, myDir, sidestepDir);
      me.go();
      recordMove(sidestepDir);
      return;
    }
  }

  if (next) {
    var dir = directionTo(myPos, next);
    recordMove(dir);
...
// === v11 unpredictability state ===

var __recentDirs = []; // persists across onIdle calls if engine keeps globals; safely empty otherwise

function recordMove(dir) {
  __recentDirs.push(dir);
  if (__recentDirs.length > 6) __recentDirs.shift();
}

function shouldBreakStraightLine(myPos, myDir, next, map, enemyTank, enemy) {
  // only juke when: BFS wants forward continuation, we've moved same dir 3+ times,
  // enemy is OUT of our LOS (so we can't see their bullets coming), and a safe juke exists
  if (!__recentDirs || __recentDirs.length < 3) return false;
  var nextDir = directionTo(myPos, next);
  if (nextDir !== myDir) return false; // BFS already turning us — no juke needed
  var tail = __recentDirs.slice(-3);
  if (!(tail[0] === myDir && tail[1] === myDir && tail[2] === myDir)) return false;
  // if enemy is visible and aligned with our column/row, the regular checks handle it
  // skip juke if enemy can see us (no blind-fire concern)
  if (enemyTank && enemyTank.position) {
    if (enemyTank.position[0] === myPos[0] || enemyTank.position[1] === myPos[1]) return false;
  }
  // skip if there is already an enemy bullet in our LOS (we'll react via dodge)
  if (enemy && enemy.bullet) return false;
  return true;
}

function pickJukeDir(myPos, myDir, map, enemyTank) {
  var perps = (myDir === "up" || myDir === "down") ? ["left", "right"] : ["up", "down"];
  for (var i = 0; i < perps.length; i++) {
    var d = perps[i];
    var p = [myPos[0] + delta(d)[0], myPos[1] + delta(d)[1]];
    if (!isOpen(p, map)) continue;
    if (enemyTank && enemyTank.position && p[0] === enemyTank.position[0] && p[1] === enemyTank.position[1]) continue;
    return d;
  }
  return null;
}
...
function queueFace(me, myDir, targetDir) {
  if (myDir === targetDir) return;
  if (myDir === "up" && targetDir === "down") { me.turn("right"); me.turn("right"); return; }
  if (myDir === "down" && targetDir === "up") { me.turn("right"); me.turn("right"); return; }
  if (myDir === "left" && targetDir === "right") { me.turn("right"); me.turn("right"); return; }
  if (myDir === "right" && targetDir === "left") { me.turn("right"); me.turn("right"); return; }
