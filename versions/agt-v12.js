...
function sidestep(me, myDir, myPos, bulletDir, map) {
  ...
  // v12: among options of equal turn cost, prefer grass ('o') — it cloaks us from enemy LOS
  function tileType(d) {
    var p = add(myPos, delta(d));
    return (map[p[0]] && map[p[0]][p[1]]) || "x";
  }
  function isGrass(d) { return tileType(d) === "o"; }
  function turnCost(d) {
    if (d === myDir) return 0;
  ...
  perps.sort(function (a, b) {
    var t = turnCost(a) - turnCost(b);
    if (t !== 0) return t;
    // tie-breaker: grass beats open ground
    return (isGrass(b) ? 1 : 0) - (isGrass(a) ? 1 : 0);
  });
