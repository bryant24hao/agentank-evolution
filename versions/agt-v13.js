function onIdle(me, enemy, game) {
  ...
  // v13: detect enemy disappearance (cloak / entered grass) — they're setting up an ambush
  var enemyVis = !!(enemyTank && enemyTank.position);
  var justLostEnemy = false;
  if (typeof __lastEnemyVisible !== 'undefined') {
    if (__lastEnemyVisible && !enemyVis) {
      __enemyLostAtFrame = game.frames;
    }
    if (typeof __enemyLostAtFrame !== 'undefined' && !enemyVis && game.frames - __enemyLostAtFrame < 3) {
      justLostEnemy = true;
    }
  }
  __lastEnemyVisible = enemyVis;
  ...
  // v13: if enemy just cloaked AND BFS wants us to continue straight, sidestep instead
  if (justLostEnemy && next && directionTo(myPos, next) === myDir) {
