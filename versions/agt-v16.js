function onIdle(me, enemy, game) {
  ...
  // v16: track enemy last-known position; avoid stepping into their lane shortly after they cloak/hide
  if (enemyTank && enemyTank.position) {
    __lastEnemyPos = [enemyTank.position[0], enemyTank.position[1]];
    __lastEnemyDir = enemyTank.direction;
    __lastSeenFrame = game.frames;
  }
  ...
  // v16: if enemy recently hid (cloak/grass) and next step puts us in their last row/col, sidestep
  if (next && !enemyTank && typeof __lastEnemyPos !== 'undefined'
      && typeof __lastSeenFrame !== 'undefined'
      && game.frames - __lastSeenFrame >= 1 && game.frames - __lastSeenFrame <= 6) {
    var sharesLane = next[0] === __lastEnemyPos[0] || next[1] === __lastEnemyPos[1];
    var inSameLaneAlready = myPos[0] === __lastEnemyPos[0] || myPos[1] === __lastEnemyPos[1];
    if (sharesLane && !inSameLaneAlready) {
      // about to step into the lane enemy was last firing from — sidestep instead
      ...
      for (var pi = 0; pi < perps.length; pi++) {
        ...
        if (isOpen(pp, map)) {
          // avoid sidestepping INTO the dangerous lane
          if (pp[0] === __lastEnemyPos[0] || pp[1] === __lastEnemyPos[1]) continue;
          if (myDir === pd) me.go();
