function onIdle(me, enemy, game) {
  ...
  // v17: maintain enemy position history for velocity inference
  if (typeof __enemyHist === 'undefined') __enemyHist = [];
  if (enemyTank && enemyTank.position) {
    __enemyHist.push({ pos: [enemyTank.position[0], enemyTank.position[1]], dir: enemyTank.direction, frame: game.frames });
    if (__enemyHist.length > 6) __enemyHist.shift();
  }
  ...
  // v17: predictive shot — if enemy moving consistently and predicted position aligns with us,
  //      fire at predicted position. Use Overload if available to widen the impact.
  if (enemyTank && canFireNow(me)) {
    var pred = predictEnemyPos(2);
    if (pred && (pred[0] === myPos[0] || pred[1] === myPos[1])) {
      var predAim = directionTo(myPos, pred);
      if (myDir === predAim && hasClearShotV17(myPos, pred, map)) {
        // we're already aimed at predicted position; fire with overload if ready
        if (me.skill && me.skill.type === "overload" && me.skill.remainingCooldownFrames === 0 && !(me.status && me.status.overloaded)) {
          var d = Math.abs(pred[0] - myPos[0]) + Math.abs(pred[1] - myPos[1]);
          if (d >= 3) me.overload();
        }
        me.fire();
        return;
...
function predictEnemyPos(framesAhead) {
  // estimate enemy position N frames in the future based on recent movement
  if (!__enemyHist || __enemyHist.length < 2) return null;
  var last = __enemyHist[__enemyHist.length - 1];
  var prev = __enemyHist[__enemyHist.length - 2];
  var dx = last.pos[0] - prev.pos[0];
  var dy = last.pos[1] - prev.pos[1];
  // only predict for steady movement (1 tile per frame)
  if (Math.abs(dx) + Math.abs(dy) !== 1) return null;
  return [last.pos[0] + dx * framesAhead, last.pos[1] + dy * framesAhead];
}

function hasClearShotV17(a, b, map) {
  if (a[0] !== b[0] && a[1] !== b[1]) return false;
  var dir = directionTo(a, b);
  var step = delta(dir);
  var pos = [a[0] + step[0], a[1] + step[1]];
  while (pos[0] !== b[0] || pos[1] !== b[1]) {
    var t = map[pos[0]] && map[pos[0]][pos[1]];
    if (t === undefined || t === "x" || t === "m") return false;
