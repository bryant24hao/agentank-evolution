function onIdle(me, enemy, game) {
  ...
  // v22: mound-breaking — if aligned with enemy and EXACTLY 1 mound blocks, fire to destroy
  // (cheap check, single linear scan)
  if (enemyTank && canFireNow(me) && !enemyBullet) {
    if (singleMoundBlockLean(myPos, enemyTank.position, map)) {
      var aimDir = directionTo(myPos, enemyTank.position);
      if (myDir === aimDir) {
        me.fire();
        return;
...
function singleMoundBlockLean(a, b, map) {
  if (a[0] !== b[0] && a[1] !== b[1]) return false;
  var sx = a[0]===b[0] ? 0 : (b[0]>a[0]?1:-1);
  var sy = a[1]===b[1] ? 0 : (b[1]>a[1]?1:-1);
  var x = a[0]+sx, y = a[1]+sy;
  var mounds = 0;
  while (x !== b[0] || y !== b[1]) {
    var t = map[x] && map[x][y];
    if (!t || t === "x") return false;
    if (t === "m") { mounds++; if (mounds > 1) return false; }
    x += sx; y += sy;
  }
  return mounds === 1;
}
