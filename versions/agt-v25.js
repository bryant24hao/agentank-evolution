...
function nextStep(start, goal, map, enemyTank, enemy, avoidLOS) {
  ...
  var seen = {};
  seen[key(start)] = true;
  // v25: dynamic BFS budget = mapSize/1.5 (min 50)
  // smaller budget → faster runtime → wins more mutual-kill tiebreakers
  var mapSize = map.length * (map[0] ? map[0].length : 0);
  var maxNodes = Math.max(50, Math.floor(mapSize / 1.5));
